...
public interface GroupControllerDocs {
    ...
    @Operation(summary = "동아리 가입 요청(Order 생성)", description = "동아리 가입을 위해 Order를 생성합니다.")
    @ApiResponse(responseCode = "200", description = "동아리 가입 요청 성공", useReturnTypeSchema = true)
    ClubJoinOrderResponse createClubJoinOrder(Long groupId, ClubJoinOrderRequest request);
